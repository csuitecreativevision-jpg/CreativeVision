  ...
  return (
    ...
    >
      ...
      <motion.nav
        ...
        className="relative z-40 w-full p-6 md:p-10 flex justify-start items-center"
      >
        <button
          onClick={onBack}
      ...
      {/* Main Content */}
      <main className="flex-1 flex flex-col z-10 px-4 md:px-10 pb-20 max-w-4xl mx-auto w-full mt-2 md:mt-4">

        {/* Header */}
        <div className="flex flex-col items-center text-center mb-12 mt-4 md:mt-6">
          <motion.div
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
          ...
          >
            ...
            <h1 className="font-display text-4xl md:text-5xl lg:text-6xl font-normal tracking-tight leading-[1.1] mb-10 text-glow-premium">
              ...
              {steps.map((step, index) => {
                ...
                return (
                  <div key={step.id} className="relative z-10 flex flex-col items-center gap-2">
                    ...
                    <span className={`absolute -bottom-6 text-[10px] font-medium tracking-wider uppercase whitespace-nowrap transition-colors duration-500 hidden md:block ${
        ...
        <motion.div
          ...
          className="w-full bg-white/[0.01] backdrop-blur-2xl border border-white/[0.05] rounded-[2rem] p-6 md:p-12 shadow-2xl relative overflow-hidden"
        >
          {/* Subtle inner glow */}
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {specializations.map((option) => (
                          <div
                            ...
                            className={`group relative flex items-start gap-4 p-6 rounded-2xl border transition-all duration-500 cursor-pointer text-left overflow-hidden ${
                          ...
                          >
                            <div className={`relative w-12 h-12 rounded-xl border flex items-center justify-center shrink-0 transition-all duration-500 ${
                              formData.specialization === option.id
                                ? 'bg-violet-500/20 border-violet-500/50'
                                : 'bg-white/5 border-white/10 group-hover:bg-white/10'
                            }`}>
                              <option.icon className={`w-5 h-5 transition-all duration-500 group-hover:scale-110 group-hover:-rotate-6 ${
                                formData.specialization === option.id ? 'text-violet-400' : 'text-white/60 group-hover:text-white'
                              }`} strokeWidth={1.5} />
                            </div>

                            <div className="flex-1 pt-0.5">
                              <h3 className={`text-lg font-bold mb-1 transition-colors duration-500 ${
                                formData.specialization === option.id ? 'text-white' : 'text-white/80 group-hover:text-white'
                              }`}>
                                {option.title}
                              </h3>
...
const RadioOption = ({ label, value, checked, onChange }: { label: string; value: string; checked: boolean; onChange: () => void }) => (
  <label className={`flex items-center gap-2 px-5 py-2.5 rounded-full border cursor-pointer transition-all ${
    checked
