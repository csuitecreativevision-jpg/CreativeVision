...
interface PortalWorkspaceCardProps {
    name: string;
    itemCount?: number;
    onClick: () => void;
    color?: string;
...
export const PortalWorkspaceCard = ({
    name,
    itemCount = 0,
    onClick,
    color = '#8b5cf6',
    index = 0,
}: PortalWorkspaceCardProps) => {
        ...
        >
            <div className="flex items-center gap-4 px-5 py-4 bg-white/[0.02] hover:bg-white/[0.045] border border-white/[0.06] hover:border-violet-500/25 rounded-2xl transition-all duration-200 hover:translate-x-0.5 hover:shadow-[0_4px_24px_rgba(139,92,246,0.07)]">
                ...
                <div className="w-8 h-8 rounded-xl bg-white/[0.04] border border-white/[0.06] flex items-center justify-center flex-shrink-0 group-hover:bg-white/[0.07] transition-colors duration-200">
                    <Users className="w-3.5 h-3.5 text-white/35 group-hover:text-white/60 transition-colors duration-200" />
                </div>

                {/* Name + label */}
                <div className="flex-1 min-w-0">
                    <div className="text-[13px] font-semibold text-white/80 group-hover:text-white truncate transition-colors duration-200 leading-tight">
                        {displayName}
                    </div>
                    <div className="text-[10px] text-white/25 font-medium mt-0.5">Active workspace</div>
                </div>
